...
<template>
    <div class="world" @mousemove="handleMouseMove" @keydown="handleKeyDown" @keyup="handleKeyUp" tabindex="0">
      ...
      <enhanced-mobile-controls 
        v-if="gameStarted && !gameOver && !gamePaused" 
        :paused="gamePaused" 
        :boost-available="canBoost"
        :boost-cooldown-percent="boostCooldownPercent"
        :is-boost-active="isBoosting"
        :has-shield="isShielded"
        :is-shield-active="isShieldActive"
        :sound-muted="audioManager && audioManager.muted"
        @move-left="onMoveLeft" 
        @move-right="onMoveRight" 
        @stop-moving="onStopMoving"
        @activate-boost="onActivateBoost" 
        @activate-shield="onActivateShield"
        @toggle-pause="pauseGame"
  ...
  .score, .coins, .high-score, .difficulty-level {
    font-size: 24px;
    margin-bottom: 10px;
  }
  ...
  .start-button, .performance-button {
  ...
  .start-button:hover, .performance-button:hover {
  ...
  @media (max-width: 768px) {
    ...
    .pause-screen h2, .game-over h2 {
      font-size: 28px;
    }
  
    .pause-screen button, .game-over button {
      padding: 12px 24px;
      font-size: 14px;
      margin: 10px 5px;
    }
