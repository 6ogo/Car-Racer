...
<template>
    <div class="world" @mousemove="handleMouseMove" @keydown="handleKeyDown" @keyup="handleKeyUp" tabindex="0">
        ...
        <div v-if="showStartScreen" class="start-screen">
            ...
            <button @click="showCarSelection">Start Game</button>
        ...
        <CarSelection v-else @car-selected="handleCarSelected" />
...
export default {
    ...
    data() {
        return {
            ...
            camera: null,
            renderer: null,
            vehicleLoader: new VehicleLoader(),
            cars: [
                { name: 'Sports Car', modelFile: 'sports_car' },
                { name: 'Sedan', modelFile: 'sedan' },
                { name: 'Monster Truck', modelFile: 'monster_truck' },
                { name: 'Roadster', modelFile: 'roadster' },
                { name: 'SUV', modelFile: 'suv' },
                { name: 'Muscle Car', modelFile: 'muscle_car' },
                { name: 'Convertible', modelFile: 'convertible' },
                { name: 'Limo', modelFile: 'limo' },
                { name: 'Coupe', modelFile: 'coupe' },
            ],
    ...
    methods: {
        ...
        preloadVehicles() {
            if (this.cars && Array.isArray(this.cars)) {
                this.vehicleLoader.preloadVehicles(
                    this.cars.map(car => car.modelFile),
                    () => console.log('All vehicles preloaded')
                );
            } else {
                console.error('Cars array is not defined or not an array');
            }
        },
        ...
        initGame() {
            ...
            this.vehicleLoader.loadVehicle(this.selectedCar.modelFile, (model) => {
                this.vehicleLoader.setVehicleColor(model, parseInt(this.selectedCar.color.replace('#', '0x')));
